...
export const expandFetchedData = (props: FetchProps, data: DoraRecord[]) => {
  data.forEach(record => {
    ...
    if (record.failed_at) {
      ...
      record.recoverTime = parseFloat(
        ((fixedAt.getTime() - failedAt) / millisecondsToHours).toFixed(2),
      );
    }

    record.created_at = record.created_at
      ? utcDateToLocal(record.created_at, false)
      : record.created_at;
    record.merged_at = record.merged_at
      ? utcDateToLocal(record.merged_at, false)
      : record.merged_at;
    record.fixed_at = record.failed_at
      ? utcDateToLocal(fixedAt, false)
      : fixedAt;
    record.failed_at = record.failed_at
      ? utcDateToLocal(record.failed_at, false)
      : record.failed_at;
  });
};
