...
const EditPanel: React.FC<Props> = (props: Props) => {
  ...
  const makeSplitter = () => {
    return <div className="editorSplit"></div>;
  };

  const childrenWithSeparator = children.reduce<React.ReactNode[]>(
    (acc, child, index) => {
      ...
      acc.push(
        <div key={uuidv4()} className="editorFieldContainer">
          {child}
        </div>,
      );
        ...
        {props.showStandardFields && (
          <>
            <div className="editorFieldContainer">
              ...
              <input
                type="text"
                value={props.args.message ?? ''}
                onChange={props.args.changeMessage}
              />
            </div>
            {makeSplitter()}
            <div className="editorFieldContainer">
              ...
              <input
                type="checkbox"
                checked={props.args.loading}
                onChange={props.args.changeLoading}
              />
            </div>
            <div className="editorFieldContainer">
              ...
              <input
                type="checkbox"
                checked={props.args.includeWeekends}
                onChange={props.args.changeIncludeWeekends}
              />
            </div>
            {makeSplitter()}
            {/* todo
